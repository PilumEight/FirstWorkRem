...
def diff():
    ...
    sumsquad = sum(list(map(lambda x: x ** 2, range(0, 101))))      #Сумма квадратов
    squadsum = sum(range(0, 101)) ** 2                              #Квадрат суммы
    result = sumsquad - squadsum
    return result * (-1)
...
def max_five():
    ...
    numerals = list(map(lambda n: int(n), filter(lambda n: n != '\n', x)))                                         #возвращаем лист без '\n' и с типом элементов int

    composition = lambda x, y, z, g, v: (x * y * z * g * v)
    result = max(list(map(composition, numerals[0:], numerals[1:], numerals[2:], numerals[3:], numerals[4:])))      #создаём список из произведения пяти последовательных цифр и ищем его максимальный член
    return result
...
def summm():
    number = reduce(lambda x, y: x ** y, [2, 1000])     #получили число по основанию 2 и степенью 1000
    result = sum(list(map(lambda x: int(x), str(number))))   #получили список цифр этого числа и получили его сумму
    return result
...
def factorial():
     result = reduce(lambda x, y: x * y, list(range(100, 0, -1)))
